...
#error XNHUD is ARC only. Either turn on ARC for the project or use -fobjc-arc flag
#endif

#import "XNHUD.h"
#import "XNIndefiniteAnimatedView.h"
#import "XNProgressAnimatedView.h"
#import "XNRadialGradientLayer.h"

NSString * const XNHUDDidReceiveTouchEventNotification = @"XNHUDDidReceiveTouchEventNotification";
NSString * const XNHUDDidTouchDownInsideNotification = @"XNHUDDidTouchDownInsideNotification";
NSString * const XNHUDWillDisappearNotification = @"XNHUDWillDisappearNotification";
NSString * const XNHUDDidDisappearNotification = @"XNHUDDidDisappearNotification";
NSString * const XNHUDWillAppearNotification = @"XNHUDWillAppearNotification";
NSString * const XNHUDDidAppearNotification = @"XNHUDDidAppearNotification";

NSString * const XNHUDStatusUserInfoKey = @"XNHUDStatusUserInfoKey";

static const CGFloat XNHUDParallaxDepthPoints = 10;
static const CGFloat XNHUDUndefinedProgress = -1;
static const CGFloat XNHUDDefaultAnimationDuration = 0.15;
...
@property (nonatomic, strong) XNProgressAnimatedView *ringView;
@property (nonatomic, strong) XNProgressAnimatedView *backgroundRingView;
...
- (XNProgressAnimatedView*)ringView;
- (XNProgressAnimatedView*)backgroundRingView;
...
+ (void)setDefaultStyle:(XNHUDStyle)style {
    [self sharedView].defaultStyle = style;
}

+ (void)setDefaultMaskType:(XNHUDMaskType)maskType {
    [self sharedView].defaultMaskType = maskType;
}

+ (void)setDefaultAnimationType:(XNHUDAnimationType)type {
...
+ (void)showWithMaskType:(XNHUDMaskType)maskType {
    XNHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
...
+ (void)showWithStatus:(NSString*)status {
    [self sharedView];
    [self showProgress:XNHUDUndefinedProgress status:status];
}

+ (void)showWithStatus:(NSString*)status maskType:(XNHUDMaskType)maskType {
    XNHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showProgress:(float)progress maskType:(XNHUDMaskType)maskType {
    XNHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showProgress:(float)progress status:(NSString*)status maskType:(XNHUDMaskType)maskType {
    XNHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showInfoWithStatus:(NSString*)status maskType:(XNHUDMaskType)maskType {
    XNHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showSuccessWithStatus:(NSString*)status maskType:(XNHUDMaskType)maskType {
    XNHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showErrorWithStatus:(NSString*)status maskType:(XNHUDMaskType)maskType {
    XNHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showImage:(UIImage*)image status:(NSString*)status maskType:(XNHUDMaskType)maskType {
    XNHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
- (instancetype)initWithFrame:(CGRect)frame {
    if((self = [super initWithFrame:frame])) {
        ...
        // Set default values
        _defaultMaskType = XNHUDMaskTypeNone;
        _defaultStyle = XNHUDStyleLight;
        _defaultAnimationType = XNHUDAnimationTypeFlat;
        ...
        NSBundle *bundle = [NSBundle bundleForClass:[XNHUD class]];
        NSURL *url = [bundle URLForResource:@"XNHUD" withExtension:@"bundle"];
        ...
        _fadeInAnimationDuration = XNHUDDefaultAnimationDuration;
        _fadeOutAnimationDuration = XNHUDDefaultAnimationDuration;
        
        // Accessibility support
        self.accessibilityIdentifier = @"XNHUD";
        self.accessibilityLabel = @"XNHUD";
...
#pragma clang diagnostic pop
	...
	if(string) {
        if(self.defaultAnimationType == XNHUDAnimationTypeFlat) {
            XNIndefiniteAnimatedView *indefiniteAnimationView = (XNIndefiniteAnimatedView*)self.indefiniteAnimatedView;
        ...
        if(self.progress != XNHUDUndefinedProgress) {
...
- (void)updateMask {
    ...
    switch (self.defaultMaskType) {
        case XNHUDMaskTypeCustom:
        case XNHUDMaskTypeBlack:{
            
            self.backgroundLayer = [CALayer layer];
            self.backgroundLayer.frame = self.bounds;
            self.backgroundLayer.backgroundColor = self.defaultMaskType == XNHUDMaskTypeCustom ? self.backgroundLayerColor.CGColor : [UIColor colorWithWhite:0 alpha:0.4].CGColor;
        ...
        case XNHUDMaskTypeGradient:{
            XNRadialGradientLayer *layer = [XNRadialGradientLayer layer];
...
- (void)updateBlurBounds {
    ...
    if(NSClassFromString(@"UIBlurEffect") && self.defaultStyle != XNHUDStyleCustom) {
        ...
        if(self.backgroundColor != [UIColor clearColor]) {
            // Create blur effect
            UIBlurEffectStyle blurEffectStyle = self.defaultStyle == XNHUDStyleDark ? UIBlurEffectStyleDark : UIBlurEffectStyleLight;
...
- (void)updateMotionEffectForXMotionEffectType:(UIInterpolatingMotionEffectType)xMotionEffectType yMotionEffectType:(UIInterpolatingMotionEffectType)yMotionEffectType {
    if([self.hudView respondsToSelector:@selector(addMotionEffect:)]) {
        ...
        effectX.minimumRelativeValue = @(-XNHUDParallaxDepthPoints);
        effectX.maximumRelativeValue = @(XNHUDParallaxDepthPoints);
        
        UIInterpolatingMotionEffect *effectY = [[UIInterpolatingMotionEffect alloc] initWithKeyPath:@"center.y" type:yMotionEffectType];
        effectY.minimumRelativeValue = @(-XNHUDParallaxDepthPoints);
        effectY.maximumRelativeValue = @(XNHUDParallaxDepthPoints);
...
#else
        // If XNHUD ist used inside an app extension add it to the given view
...
- (NSDictionary*)notificationUserInfo{
    return (self.statusLabel.text ? @{XNHUDStatusUserInfoKey : self.statusLabel.text} : nil);
}
...
- (void)positionHUD:(NSNotification*)notification {
    ...
    if(notification) {
        ...
        __weak XNHUD *weakSelf = self;
        [UIView animateWithDuration:animationDuration
                              delay:0
                            options:UIViewAnimationOptionAllowUserInteraction
                         animations:^{
                             __strong XNHUD *strongSelf = weakSelf;
                             if(strongSelf) {
...
- (void)overlayViewDidReceiveTouchEvent:(id)sender forEvent:(UIEvent*)event {
    [[NSNotificationCenter defaultCenter] postNotificationName:XNHUDDidReceiveTouchEventNotification
                                                        object:self
    ...
    if(CGRectContainsPoint(self.hudView.frame, touchLocation)) {
        [[NSNotificationCenter defaultCenter] postNotificationName:XNHUDDidTouchDownInsideNotification
                                                            object:self
                                                          userInfo:[self notificationUserInfo]];
    }
}
...
- (void)showImage:(UIImage*)image status:(NSString*)status duration:(NSTimeInterval)duration {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            ...
            strongSelf.progress = XNHUDUndefinedProgress;
            [strongSelf cancelRingLayerAnimation];
            [strongSelf cancelIndefiniteAnimatedViewAnimation];
...
- (void)showStatus:(NSString*)status {
    ...
    if(self.defaultMaskType != XNHUDMaskTypeNone) {
    ...
    if(self.alpha != 1.0f || self.hudView.alpha != 1.0f) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:XNHUDWillAppearNotification
                                                            object:self
                                                          userInfo:[self notificationUserInfo]];
        ...
        __block void (^completionBlock)(void) = ^{
            ...
            if(strongSelf) {
                ...
                // Post notification to inform user
                [[NSNotificationCenter defaultCenter] postNotificationName:XNHUDDidAppearNotification
                                                                    object:strongSelf
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            ...
            if(strongSelf.alpha != 0.0f || strongSelf.hudView.alpha != 0.0f){
                // Post notification to inform user
                [[NSNotificationCenter defaultCenter] postNotificationName:XNHUDWillDisappearNotification
                                                                    object:nil
                                                                  userInfo:[strongSelf notificationUserInfo]];
                ...
                __block void (^completionBlock)(void) = ^{
                    ...
                    // Post notification to inform user
                    [[NSNotificationCenter defaultCenter] postNotificationName:XNHUDDidDisappearNotification
                                                                        object:strongSelf
                                                                      userInfo:[strongSelf notificationUserInfo]];
...
- (UIView*)indefiniteAnimatedView {
    ...
    if(self.defaultAnimationType == XNHUDAnimationTypeFlat){
        ...
        if(_indefiniteAnimatedView && ![_indefiniteAnimatedView isKindOfClass:[XNIndefiniteAnimatedView class]]){
            [_indefiniteAnimatedView removeFromSuperview];
            _indefiniteAnimatedView = nil;
        }
        
        if(!_indefiniteAnimatedView){
            _indefiniteAnimatedView = [[XNIndefiniteAnimatedView alloc] initWithFrame:CGRectZero];
        }
        
        // Update styling
        XNIndefiniteAnimatedView *indefiniteAnimatedView = (XNIndefiniteAnimatedView*)_indefiniteAnimatedView;
...
- (XNProgressAnimatedView*)ringView {
    if(!_ringView) {
        _ringView = [[XNProgressAnimatedView alloc] initWithFrame:CGRectZero];
    }
...
- (XNProgressAnimatedView*)backgroundRingView {
    if(!_backgroundRingView) {
        _backgroundRingView = [[XNProgressAnimatedView alloc] initWithFrame:CGRectZero];
...
- (UIColor*)foregroundColorForStyle {
    if(self.defaultStyle == XNHUDStyleLight) {
        return [UIColor blackColor];
    } else if(self.defaultStyle == XNHUDStyleDark) {
...
- (BOOL)isClear { // used for iOS 7 and above
    return (self.defaultMaskType == XNHUDMaskTypeClear || self.defaultMaskType == XNHUDMaskTypeNone);
}
...
- (void)setDefaultAnimationType:(XNHUDAnimationType)animationType {
