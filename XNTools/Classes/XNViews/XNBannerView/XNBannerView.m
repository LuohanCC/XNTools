...
- (void)setupAutoSlide:(BOOL)on {
    ...
    if(on) {
        STOP_AUTOSLIDE();
        _timer = [NSTimer scheduledTimerWithTimeInterval:AUTO_SLIDE_INTERVAL target:self selector:@selector(slideToNaxePage) userInfo:nil repeats:YES];
    }else{
        STOP_AUTOSLIDE();
    }
}
...
//    }
    [NSURLConnection sendAsynchronousRequest:[NSURLRequest requestWithURL:url] queue:[[NSOperationQueue alloc] init] completionHandler:^(NSURLResponse *response, NSData *data, NSError *connectionError){
        if (!connectionError) {
            ...
            [_imageArray setObject:image atIndexedSubscript:index];
