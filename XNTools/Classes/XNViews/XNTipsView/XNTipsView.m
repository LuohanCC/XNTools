...
- (void)addSubviews {
    ...
    UIView *targetView = _targetView ? _targetView : XNAppDelegate.window;
...
- (void)updateSubviewFrame {
    ...
    if (XNScreenHeight > XNScreenWidth) { //竖屏
        maxDialogHeight = XNScreenHeight - XNNaviBarHeight - XNTabbarHeight - 100;
        dialogWidth = XNScreenWidth * 0.8f;
    } else {
        maxDialogHeight = XNScreenHeight * 0.8;
        dialogWidth = XNScreenWidth * 0.6f;
    }
    ...
    self.contentBackgroundView.frame = CGRectMake((XNScreenWidth-dialogWidth) / 2, (XNScreenHeight-dialogHeight)/2, dialogWidth, dialogHeight);
...
- (void)handleStatusBarOrientationChange: (NSNotification *)notification{
    if(currentScreenWidth != XNScreenWidth) {
        currentScreenWidth = XNScreenWidth;
        if(_shadowView) {
...
- (CGFloat)getDefaultWidth {
    float minxmumScreenWidth = XNScreenWidth < XNScreenHeight ? XNScreenWidth : XNScreenHeight;
...
- (CABasicAnimation *)horizontalAnimation:(BOOL)isFromLeft isFromCenterX:(BOOL)isFromCenterX {
    ...
    if(isFromLeft) {
        ...
        ePoint = CGPointMake(isFromCenterX ? XNScreenWidth+frame.size.width/2 : v.center.x , v.center.y);
    }else{
        sPoint = CGPointMake(isFromCenterX ? v.center.x : XNScreenWidth+frame.size.width/2 , v.center.y);
...
- (CABasicAnimation *)verticalAnimation:(BOOL)isLanding isFromCenterY:(BOOL)isFromCenterY {
    ...
    if(isLanding) {
        ...
        ePoint = CGPointMake(v.center.x , isFromCenterY ? XNScreenHeight+frame.size.height/2 : v.center.y);
    }else{
        sPoint = CGPointMake(v.center.x , isFromCenterY ? v.center.y : XNScreenHeight + frame.size.height/2);
